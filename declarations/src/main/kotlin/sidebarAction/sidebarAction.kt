 ...
 * @param tabId Sets the sidebar title for the tab specified by tabId. Automatically resets when the
        tab is closed.
...
/**
 * @param tabId Specify the tab to get the title from. If no tab nor window is specified, the global
        title is returned.
 * @param windowId Specify the window to get the title from. If no tab nor window is specified, the
        global title is returned.
 */
class Details2(
    var tabId: Int? = null,
    var windowId: Int? = null
 ...
 * @param path Either a relative image path or a dictionary {size -> relative image path} pointing
        ...
        foo}'
 * @param tabId Sets the sidebar icon for the tab specified by tabId. Automatically resets when the
        tab is closed.
...
/**
 * The url to the html file to show in a sidebar.  If set to the empty string (''), no sidebar is
        shown. */
typealias Panel = Any

/**
 * @param tabId Sets the sidebar url for the tab specified by tabId. Automatically resets when the
        tab is closed.
 * @param windowId Sets the sidebar url for the window specified by windowId.
 * @param panel The url to the html file to show in a sidebar.  If set to the empty string (''), no
        sidebar is shown.
 */
class Details4(
    var tabId: Int? = null,
...
/**
 * @param tabId Specify the tab to get the panel from. If no tab nor window is specified, the global
        panel is returned.
 * @param windowId Specify the window to get the panel from. If no tab nor window is specified, the
        global panel is returned.
 */
class Details5(
    var tabId: Int? = null,
    var windowId: Int? = null
...
external class SidebarActionNamespace {
    ...
    /**
     * Sets the icon for the sidebar action. The icon can be specified either as the path to an
            image file or as the pixel data from a canvas element, or as dictionary of either one of
            those. Either the <strong>path</strong> or the <strong>imageData</strong> property must
            be specified.
     */
    fun setIcon(details: Details3): Promise<Any>

    /**
     * Sets the url to the html document to be opened in the sidebar when the user clicks on the
            sidebar action's icon.
     */
